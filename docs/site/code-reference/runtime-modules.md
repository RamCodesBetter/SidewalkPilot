...
The model registry contains all 46 checkpoints. Early Series 1/2 models can use local PyTorch inference when configured. Series 3/4 run as ONNX on Jetson Orin Nano. Series 4 PC/PCF maintain causal history inside the single inference server; CF remains image-only.
