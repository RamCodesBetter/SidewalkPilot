...
- **Series 4.1:** six corrected ONNX models are trained, evaluated offline, and registered in the live selector. They have not been driven.
- **Current local Series 3/4 corpus:** 80,969 images after 268 confirmed off-domain or duplicate files were quarantined. Existing models and the frozen report still use the 81,237-image snapshot.
...
The v4.1 trainers were changed in response to the steering-echo failure. Their new losses and history perturbations still require closed-loop replay and supervised physical comparison. v3.4 and v4.0f are the controls for that later test.
...
2. Run signature and fresh-response checks for each v4.1 model before driving it.
...
- `code/controller/current/rc_car_app/vision.py`: all 52 live-selectable checkpoint names through v4.1 and the v3.4 default.
...
Next: preserve a physical AEB result, replay-test the v4.1 candidates, and promote a new model only if it improves physical driving over v3.4.
