...
| Navigation | BN880 GPS and HMC5883L compass | Position, heading, and graph-route support |
...
- **LiDAR:** USB 3.0 through CP2102, production serial device `/dev/ttyUSB0`.
...
| Hall sensor | GPIO 24 |
| LiDAR | CP2102 USB serial; no production GPIO motor-enable line |

The production source of truth is `code/controller/current/rc_car_app/config.py`. Pin changes must update the wiring document, bench test, and config together.
...
```bash
python code/test_files/pca9685_servo_test.py
python code/test_files/hall_sensor_test.py
python code/test_files/lidar_viewer.py
python code/test_files/bn880_test.py
python code/test_files/xbox_test.py
sudo python code/test_files/hub75_rgbmatrix_test.py
```

Run only one program that owns a device at a time. Stop the production car service before opening the same camera, serial port, PCA9685 channel, or GPIO from a test tool.
