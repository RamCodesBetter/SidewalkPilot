...
The display system is a Raspberry Pi Zero 2 W driving one Waveshare 64×32 RGB LED matrix through HUB75. The current production renderer does not use a MAX7219 display.
...
| `code/test_files/hub75_rgbmatrix_test.py` | Panel color/glyph bench test |
...
The receiver exposes matrix-driver settings for slowdown GPIO, row addressing, multiplexing, panel type, brightness, PWM bits, refresh limit, RGB sequence, and hardware pulse behavior. Production values belong in the Zero's systemd service rather than in page rendering code.
