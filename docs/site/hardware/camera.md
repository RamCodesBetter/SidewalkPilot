...
connected to the Pi 5 over the CSI ribbon and driven through Picamera2 in
`code/controller/current/rc_car_app/vision.py`.

## Parts (Amazon)

- [RPI Camera Module 3 Wide](https://www.amazon.com/Raspberry-Pi-Camera-Module-Wide/dp/B0BRY757NX?ref_=ast_sto_dp&th=1) — $69.99
...
- Each captured frame is resized before inference. Series 1/2 preprocessing resizes to
  `200x66`; Series 3/4 use a `320x180` input on the Jetson. Operator-enabled capture saves
...
- The module is integrated through the Raspberry Pi camera path, and the current 81,237-image Series 3/4 dataset was captured
