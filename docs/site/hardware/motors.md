...
- Keeping optional left and right motor scaling separate from steering calibration makes the two
  effects measurable instead of hiding one inside the other.

## Verify Before a Run

- Bench test: the Hall-effect wheel-speed sensor (`code/test_files/sensors/hall_sensor_test.py`) confirms wheel
