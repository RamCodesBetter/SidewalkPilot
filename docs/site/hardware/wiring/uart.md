...
| IMU | Seeed XIAO MG24 Sense (6-axis) | `/dev/ttyAMA3` (Pi GPIO8/9) | `115200` | `STEERING_YAW_PID_PORT` / `STEERING_YAW_PID_BAUD` |
...
The LiDAR was originally a GPIO-UART device on `/dev/ttyAMA2` at 230400 baud (with a Pi 5 UART overlay on GPIO4/5). It has since moved to a USB/CP2102 connection; see the USB wiring page. The `230400` baud and the FHL-LD19 packet format are unchanged, only the transport moved.
...
```bash
# Pi 5 — GPS (stop the car service first to avoid two readers)
timeout 5s cat /dev/ttyAMA0        # expect NMEA sentences ($GPGGA, $GPRMC ...)

# Pi 5 — raw LiDAR bytes when on GPIO UART (former path)
