...
| Dashboard link | Raspberry Pi Zero 2 W | USB Ethernet gadget (`usb0`) | Pi `192.168.10.1`, Zero `192.168.10.2`, UDP `8765` |
...
**Dashboard USB Ethernet.** The Pi 5 and Zero 2 W are linked by a USB Ethernet gadget that exposes `usb0` on both ends with fixed addresses: Pi `192.168.10.1/24`, Zero `192.168.10.2/24`. The Pi sends dashboard telemetry as UDP to `192.168.10.2:8765` (`HUB75_DASHBOARD_HOST` / `HUB75_DASHBOARD_UDP_PORT` in `config.py`). This link is USB-only by current project decision, with no Wi-Fi fallback. A serial fallback over `/dev/ttyACM0` at 115200 exists in code but is not the active transport.
...
```bash
# Pi 5 — LiDAR port present and readable
ls -l /dev/serial/by-id/ | grep -i cp2102
ls -l /dev/ttyUSB*

# Pi 5 — USB Ethernet link to the Zero
ip -br addr show usb0
cat /sys/class/net/usb0/carrier      # 1 = link up
ping -c 3 192.168.10.2               # from Pi 5
