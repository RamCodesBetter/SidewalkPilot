...
| LiDAR UART parsing | `LidarParser` thread | Read latest scan |
...
Both Jetson and dashboard workers are **latest-value** boundaries. If production is slower than capture, old pending work is replaced instead of building an increasingly stale queue.
