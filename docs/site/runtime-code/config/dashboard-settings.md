...
This page documents the HUB75 dashboard-telemetry constants in `code/controller/current/rc_car_app/config.py`, the Raspberry Pi 5 sender `code/controller/current/rc_car_app/hub75_dashboard.py`, and the Zero 2 W receiver/renderer `code/controller/current/z2w_dashboard.py`. The link is USB Ethernet only: the Raspberry Pi 5 sends UDP to the Zero 2 W.

## How it works

The Raspberry Pi 5 is `192.168.10.1` and the Zero 2 W is `192.168.10.2` over the `usb0` gadget. The Raspberry Pi 5 packs a small JSON payload every send interval and sends it by UDP to `192.168.10.2:8765`; the Zero 2 W receives it and draws the current page on the LED panel.
...
| `HUB75_DASHBOARD_UDP_PORT` | `8765` | UDP port the Zero 2 W receiver listens on |
...
The highest page ID is threaded through every layer. `DASHBOARD_PAGE_COUNT = 17` in `config.py`, while `DASHBOARD_PAGE_COORDS` defines the visible two-dimensional grid. IDs are sparse because removed pages were not renumbered. Any page change must be updated in the controller grid, serializer clamp, and Zero 2 W renderer.
...
`NO LINK` on the LED panel means the Zero 2 W receiver process is alive but has not received a packet recently — the render side is fine, the packets are not arriving. Typical causes: the Raspberry Pi 5 controller is not running, `usb0` shows carrier `1` but ARP/ping fails, or the sender never connected. On a healthy Raspberry Pi 5 boot the log prints `Hub75 dashboard telemetry sending UDP to 192.168.10.2:8765.`; if it instead prints a `zero2w.local` target, stale code or an env override is active. Debug order: car service up, Zero 2 W receiver listening on `8765`, `usb0` up with carrier, ping both ways, then read logs.
