...
The dashboard can cycle all 46 registered Series 1-4 version names while the controller process remains running. Selection behavior differs between local Pi inference and the current camera-only/Jetson path.
...
In `camera_only` mode, the Pi does not load a local checkpoint. It records the requested name, clears cached analysis, and sends the name with subsequent frames. A successful Pi-side selection proves only that the name was accepted; Jon logs and a fresh matching response are required to prove that the ONNX artifact loaded.

For Series 4 PC/PCF models, Jon resets causal steering history on load or switch so commands from the previous model do not enter the new model's input history.
...
- For Series 3/4, confirm Jon logs the matching ONNX path/provider and the Pi receives fresh results for that exact model name.
- Keep autonomy off during the switch and initial verification.

## Related pages

- [Model Choices](model-choices.md)
- [Jetson Inference Link](../../autonomy-stack/camera-steering/jetson-inference-link.md)
