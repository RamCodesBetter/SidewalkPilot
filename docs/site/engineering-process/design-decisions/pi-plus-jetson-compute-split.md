...
- The **Jetson Orin Nano** runs every live-selectable steering-model family on its GPU:
  Series 1/2 through PyTorch CUDA and Series 3/4 through ONNX Runtime CUDA. It returns
  decoded steering plus runtime telemetry over private Ethernet at `10.42.0.2:8770`.
...
- The Raspberry Pi 5 captures the frame through **Picamera2** in OpenCV BGR
  ...
  Jetson Orin Nano performs model-specific resize/normalization, runs the selected model, decodes the
...
- **Speed motivation:** Series 1/2 (`SteeringAutonomyV2`, ~0.67M parameters, 200x66)
  are smaller than the ~5.5M-parameter, 320x180 Series 3/4 networks, but all live-selectable
  families use the same Jetson Orin Nano inference service. This leaves the Raspberry Pi 5 CPU
  available for physical I/O while the GPU handles inference. Exact processing rate should still be reported
