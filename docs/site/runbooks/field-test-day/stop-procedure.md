...
- The controller is connected, the Pi loop is responsive, and the operator has a physical power-stop option.
...
4. Confirm linked shutdown reached the Zero. `dashboard_sender.send_shutdown()` tells the receiver to stop, so the Zero display should go dark or show its shutdown state rather than sitting on stale telemetry. If it instead shows `NO LINK`, the receiver is alive but stopped getting packets — expected once the car quits.
...
- Zero dashboard in shutdown state
