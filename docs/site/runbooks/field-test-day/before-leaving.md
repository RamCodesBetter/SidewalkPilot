...
- The steering model to run is decided (see `model-selection.md`). Series 1/2 checkpoints in `code/ai_models/` run on the Pi; Series 3/4 ONNX models run on the Jetson (Jon, `10.42.0.2:8770`). v3.4 is the current field-selected baseline. Series 4 models require field testing before promotion.
- The latest code is on the Pi. The Pi's `~/rc_car_code` is the git repo; the Zero's copy is synced by rsync/scp and is not a repo, so a code change only takes effect after each device re-imports it.

## Steps

1. Charge and pack the 3S LiPo that powers the drive motors, plus the Pi/electronics power. Do the full battery inspection in `battery-check.md` before loading anything. Evidence: measured pack voltage.
...
4. Confirm the RPi Camera Module 3 Wide is seated and its ribbon is locked. The current camera integration and all 81,237 Series 3/4 images use this Pi camera path; a loose ribbon breaks capture and model input.
5. Pack the Zero 2 W dashboard and the USB Ethernet cable. The dashboard is USB-only: Pi `usb0` is `192.168.10.1`, Zero `usb0` is `192.168.10.2`, telemetry is UDP to `192.168.10.2:8765`. Use the known working port and power arrangement; `-110`/`-62` enumeration errors indicate a failed USB transaction but do not by themselves prove whether the cause is power, cable, port, or gadget state.
6. Clear space on the Pi for the run's output. Each run writes a CSV to `~/logs/log_YYYYMMDD_HHMMSS.csv` by default (or under `RC_CAR_LOG_DIR`) and photos to `media/photos/YYYY_MM_DD_run_N/` with a per-run JSON label file. Confirm free disk in both locations.
