...
3. Confirm the sensor harness is intact: LiDAR (Youyeetoo FHL-LD19, USB `/dev/ttyUSB0` via CP2102, 230400 baud), GPS (BN880 on `/dev/ttyAMA0`, 9600), hall sensor (GPIO24), servo ribbon to the PCA9685 (I2C `0x40`, channel 0), and the JGB37-520 DC motor leads through the AT8236 (GPIO 19/20 right, 25/13 left). Evidence: visual/tug check of each connector.
