...
Battery Check is the power runbook run before every field test. SidewalkPilot has two power domains: a 3S LiPo drives the motors through the Yahboom AT8236 H-bridge, and a separate supply powers the Pi 5, camera, and sensors. A sagging pack shows up as weak or asymmetric drive thrust and can be mistaken for a steering or model problem, so this check comes first. Follow it in order; each step ends with a measured value or a pass/fail.
...
3. Confirm the electronics supply (Pi 5 + camera + sensors) is separate from the motor pack and adequately charged. The Pi browns out under camera and inference load if the supply is weak.
4. Power the car on and confirm a clean boot with no undervoltage warnings. On the Pi, CPU temperature is read from `vcgencmd measure_temp` and surfaced in the CSV log (`CPU Temp (C)`) and on the dashboard; a throttled Pi is often a power symptom.
...
Abort the run if the LiPo is puffed or below a safe field voltage, if per-cell balance is far off, if the Pi reports undervoltage/throttling, or if one drive channel does not respond. A weak pack invalidates any steering or model conclusions drawn from the run.
