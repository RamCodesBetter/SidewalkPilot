...
Pulling PC-to-Mac follows the same rules: dry-run, scope to the named artifact/run, and omit `--delete`. A whole-repository reverse mirror can remove docs, datasets, or photos absent on the remote. Source code should return through Git; model/data artifacts should return through an explicit path.
