...
This page describes what data SidewalkPilot collects, where it is stored, and how privacy is handled. The car carries a forward camera (RPi Camera Module 3 Wide) and a GPS, so it captures images of real sidewalks and, potentially, of people, houses, and vehicles nearby. The project treats that as a real privacy responsibility, not an afterthought.
...
Field capture stays local to the vehicle. Photos are written under `media/photos/YYYY_MM_DD_run_N/` (`create_photo_run_dir`), one folder per run, named `photo_<timestamp>.jpg`. CSV logs are written under `~/logs` by default. Camera frames used for remote inference travel over the private Pi-to-Jetson Ethernet link; dashboard state travels over the private Pi-to-Zero USB Ethernet link. The runtime does not upload field images to a cloud service. Optional InfluxDB telemetry is numeric vehicle state and is configured separately through `~/.influxdb.json`.
...
- **Local-first.** Images live on the Pi until Ram intentionally pulls them to the Mac workstation for training-set curation. The documented sync guidance deliberately avoids destructive whole-repo mirroring so field data is not accidentally deleted or duplicated.
...
Series 3 and Series 4 send frames from the Pi to the Jetson (Jon) at `10.42.0.2:8770` for inference. That local transfer is not itself a storage or publication path. Images are persisted only through the Pi's operator-controlled photo/run-capture flow. Any future inference-frame logging on Jon would be a new data path and should be reviewed against this page before use.
