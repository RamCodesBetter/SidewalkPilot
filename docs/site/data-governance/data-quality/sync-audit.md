...
- Good: `rsync -a` (no `--delete`) of just `media/photos/2026_07_02_run_1/` from Pi to Mac, then a file-count diff confirming source and destination match.
...
Compare counts across a link (label paths not present on the destination are the leak indicator). On the Pi source:
