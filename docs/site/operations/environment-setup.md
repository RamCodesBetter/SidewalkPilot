...
How each SidewalkPilot machine is prepared before it can run its part of the stack: the Pi 5 controller, the Zero 2 W dashboard receiver, the Jetson (Jon) inference host, and the Mac/PC workstations. The goal is a repeatable setup so a fresh device can be brought online without guessing at paths or ports.
...
|---|---|---|
| Pi 5 | Controller: Xbox controller, camera, LiDAR, GPS, hall sensor, servo, motors, logging, telemetry | Repo cloned, Python deps (`gpiozero`, `adafruit-servokit`, `picamera2`, `pyserial`, `opencv`, `torch`), USB `usb0` at `192.168.10.1/24`, I2C enabled for the PCA9685 at `0x40` |
...
| Jetson (Jon) | Series 3/4 ONNX inference host | ONNX Runtime, model `.onnx` files copied over, headless inference service; reachable at `10.42.0.2:8770` over direct Ethernet |
| Mac / NVIDIA PC | Git, sync, photo pull, training | Git access, rsync, training deps for the trainers under `code/ai_models_datasets/` |

## Why this choice

- Deriving paths from the repo location means a fresh checkout works without hand-editing constants, and the same code runs on the Pi and in a laptop simulation fallback (the hardware layer degrades to dummy devices when GPIO/I2C is absent).
...
- Fixed USB Ethernet addresses (`.1` Pi, `.2` Zero) keep dashboard routing independent of Wi-Fi and mDNS.
...
# Pi 5 / Zero: confirm the USB link addresses
ip -br addr show usb0
cat /sys/class/net/usb0/carrier
```

A healthy Pi controller prints `Hub75 dashboard telemetry transport: UDP (192.168.10.2:8765).` on startup, and the PCA9685 line `Using PCA9685 steering servo at 0x40, channel 0.` once I2C is up.
...
- **Zero boots then hangs headless**: a missing `/etc/machine-id` triggers the systemd first-boot wizard. Regenerate the machine-id.
