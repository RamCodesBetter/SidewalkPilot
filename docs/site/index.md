...
SidewalkPilot is an independently built autonomous RC-car research platform that uses a camera model to steer along sidewalks while a separate LiDAR layer can slow or stop the vehicle.
...
> SidewalkPilot is a private-route research and learning project. It is not road legal, is not approved for unattended operation, and is not presented as a finished autonomous vehicle.
...
- [Offline model evaluation](ai-and-models/model-zoo/series-3.md) compares all Series 3 checkpoints on 81,237 labeled real-world images.
...
2. The Raspberry Pi 5 sends the newest frame to the Jetson without blocking manual control.
...
SidewalkPilot progressed through three model families, multiple sensor transports, steering calibration experiments, dashboard-link failures, and repeated field-to-dataset cycles. Important lessons include:

- A low average steering error can hide a model that predicts straight too often.
- Stronger augmentation can make a model worse if it removes useful visual structure.
