...
`code/test_files/evaluate_sidewalkpilot_models.py` evaluates all 46 checkpoints on the same 6,952-frame Series 3/4 validation subset. It adapts input resolution and output decoding by series, writes `docs/steering_eval_current_labels.json`, and generates `docs/steering_model_report.pdf`.
