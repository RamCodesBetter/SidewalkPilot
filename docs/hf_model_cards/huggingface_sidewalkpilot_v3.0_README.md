...
SidewalkPilot-v3.0 is the first Series 3 checkpoint. Unlike the steering-only Series 1/2 models, it predicts **both steering and throttle** from a single `320x180` OpenCV BGR camera frame, using the heavier `SidewalkPilotV3` architecture intended for Jetson (ONNX/TensorRT) deployment.

**This checkpoint underperformed and is not field-usable.** Its steering MAE is much higher than the Series 2 models, mainly because the Series 3 training images were captured with a **tilted camera**, so the data quality was poor. It is published for history/completeness, not as a recommended model.
...
- **License:** Apache 2.0
- **Checkpoint:** `SidewalkPilot-v3.0.onnx` (FP32; the `.pth` master was removed after ONNX export)
...
- First SidewalkPilot model to jointly predict **steering AND throttle** (Series 1/2 were steering-only).
- New heavier `SidewalkPilotV3` backbone at `320x180`, exported to ONNX FP32 for Jetson ("Jon") inference.
- Establishes the Series 3 pipeline: aggregate dataset, single combined evaluator, and ONNX inference served to the Pi over the network.

## Specific Issues Observed / Remaining

- **Much worse steering MAE than Series 2** (`10.225` deg vs `3.265` deg for v2.4b). Not field-usable.
...
- **Model:** `3.0`
- **Checkpoint:** `SidewalkPilot-v3.0.onnx`
- **Checkpoint created:** 2026-06-29 America/Los_Angeles
- **Full score:** `94.319%`
