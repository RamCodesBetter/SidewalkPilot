...
from .vision import WebcamVisionProcessor
...
def print_controls():
    ...
    print("  D-pad up/down: adjust cruise speed, or display brightness when cruise is off")
    print(f"  Right stick Y (axis {DASHBOARD_PAGE_AXIS}): hold up/down for dashboard pages")
...
def update_dashboard_page_selection(state, metrics) -> None:
    axis_value = float(state.get("dashboard_page_axis_value", 0.0))
    if axis_value >= DASHBOARD_PAGE_AXIS_THRESHOLD:
        direction = 1
    elif axis_value <= -DASHBOARD_PAGE_AXIS_THRESHOLD:
        direction = -1
    else:
        metrics.dashboard_page_axis_direction = 0
        metrics.dashboard_page_axis_hold_since = 0.0
        metrics.dashboard_page_axis_latched = False
        return

    now = time.monotonic()
    if direction != metrics.dashboard_page_axis_direction:
        metrics.dashboard_page_axis_direction = direction
        metrics.dashboard_page_axis_hold_since = now
        metrics.dashboard_page_axis_latched = False
        return

    if metrics.dashboard_page_axis_latched:
        return

    if now - metrics.dashboard_page_axis_hold_since < DASHBOARD_PAGE_HOLD_SEC:
        return

    current_page = max(1, min(DASHBOARD_PAGE_COUNT, int(state.get("dashboard_page", 1))))
    next_page = ((current_page - 1 + direction) % DASHBOARD_PAGE_COUNT) + 1
    metrics.dashboard_page_axis_latched = True
    state["dashboard_page"] = next_page
    metrics.dashboard_page_transition = "forward" if direction > 0 else "back"
...
def take_photo(webcam_vision=None):
    global current_photo_run_dir
    if current_photo_run_dir is None:
        current_photo_run_dir = create_photo_run_dir()
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = str(current_photo_run_dir / f"photo_{timestamp}.jpg")
    if webcam_vision:
...
def run(model_choice=None):
    ...
    if not lidar_parser.connect():
        print("Failed to connect to LiDAR. Running without LiDAR data.")
        lidar_parser = None

    webcam_vision = WebcamVisionProcessor(model_choice=model_choice)
        ...
        while not shutdown_flag.is_set():
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.QUIT:
                    ...
                    if hat_y == 1 and state["cc_active"]:
                    ...
                    elif hat_y == -1 and state["cc_active"]:
            ...
            camera_pixels = []
            if state["dashboard_page"] == 5 and webcam_vision is not None:
