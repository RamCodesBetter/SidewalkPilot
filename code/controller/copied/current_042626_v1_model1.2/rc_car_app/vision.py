...
STEERING_MODEL_CHOICES = {
    "1.0": "SidewalkPilot-v1.0.pth",
    "1.0b": "SidewalkPilot-v1.0b.pth",
    "1.1": "SidewalkPilot-v1.1.pth",
    "1.1b": "SidewalkPilot-v1.1b.pth",
    "1.2": "SidewalkPilot-v1.2.pth",
    "1.2b": "SidewalkPilot-v1.2b.pth",
}
...
STEERING_MODEL_WIDTH = 160
STEERING_MODEL_HEIGHT = 120
...
def preprocess_steering_frame(frame):
...
def estimate_path_bias_from_frame(frame):
    ...
    low_sat_mask = (saturation < 75).astype(np.uint8) * 255
    lit_mask = (value > 45).astype(np.uint8) * 255
    candidate_mask = cv2.bitwise_and(cv2.bitwise_and(neutral_mask, low_sat_mask), lit_mask)
...
class WebcamVisionProcessor:
    ...
    def start(self):
        ...
        try:
            ...
            self.steering_model = SteeringAutonomyV2().to(self.torch_device)
            try:
                checkpoint = torch.load(str(self.model_path), map_location=self.torch_device, weights_only=True)
            except TypeError:
                checkpoint = torch.load(str(self.model_path), map_location=self.torch_device)
            state_dict = checkpoint.get("state_dict", checkpoint) if isinstance(checkpoint, dict) else checkpoint
            self.steering_model.load_state_dict(state_dict)
    ...
    def _estimate_path_bias(self, frame):
        if self.steering_model is None or self.torch_device is None or torch is None:
            analysis = _empty_analysis()
            analysis["method"] = "steering_model_unavailable"
            return analysis
        try:
            tensor = preprocess_steering_frame(frame).to(self.torch_device)
            with torch.no_grad():
                steering_angle = float(torch.clamp(self.steering_model(tensor), 0.0, 180.0).item())
        ...
        analysis.update(
            {
                ...
                "method": f"SidewalkPilot:{self.model_choice}",
            }
        )
        return analysis
