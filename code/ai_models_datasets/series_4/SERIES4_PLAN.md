...
1. Freeze a reproducible v3.4 replay on all 81,237 Series 3 images and the current Series 3 time-window validation split.
...
## v4.0 Dataset Contract

SidewalkPilot v4.0 will train on the same 81,237-image Series 3 dataset used for the current v3.4 evaluation. It will reuse the exact frozen time-window train/validation assignment rather than generating a new random split.

The first comparison must not add, remove, relabel, or reweight examples for v4.0 alone. The purpose is to isolate the Series 4 architecture. Any later dataset expansion or label correction becomes a separate experiment and model version.
